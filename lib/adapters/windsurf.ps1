# windsurf.ps1 - Windsurf 适配器 (PowerShell)
# 安装方式：复制 SKILL.md 内容作为规则文件

$AdapterTarget = Join-Path $env:USERPROFILE ".windsurf\rules"
...
function Install-AdapterSkill {
    ...
    $header = "<!-- Skill: $Name v$version -->`n<!-- Agent: windsurf -->`n`n"
...
function Uninstall-AdapterSkill {
    ...
    if (Test-Path $dst) {
        Remove-Item $dst -Force
        Write-Success "$Name uninstalled from Windsurf"
        Write-LogOperation -Action "remove" -Src "" -Dst $dst -Success $true
        return $true
    } else {
        Write-Info "$Name not installed in Windsurf"
        return $true
    }
}
