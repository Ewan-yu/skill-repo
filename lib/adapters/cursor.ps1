# cursor.ps1 - Cursor 适配器 (PowerShell)
# 安装方式：复制 SKILL.md 内容作为规则文件

$AdapterTarget = Join-Path $env:USERPROFILE ".cursor\rules"
$AdapterName = "cursor"
...
function Install-AdapterSkill {
    ...
    # 写入规则文件
    $header = "<!-- Skill: $Name v$version -->`n<!-- Agent: cursor -->`n`n"
...
function Uninstall-AdapterSkill {
    ...
    if (Test-Path $dst) {
        Remove-Item $dst -Force
        Write-Success "$Name uninstalled from Cursor"
        Write-LogOperation -Action "remove" -Src "" -Dst $dst -Success $true
        return $true
    } else {
        Write-Info "$Name not installed in Cursor"
        return $true
    }
}
